...
##
if (req.url ~ "^/index.php?controller=order"){
     return (pass);
  }
...
##
  ...
  if (req.method == "GET" && (req.url ~ "admin" || req.url ~ "login")) {
      return (pass);
  }
  if (req.method == "GET" && (req.url ~ "cart" || req.url ~ "checkout")) {
      return (pass);
  }
...
##
if (bereq.url ~ "index.php?controller=order"){
 set beresp.ttl = 0s;
...
##
  ...
  if (bereq.method == "GET" && (bereq.url ~ "admin" || bereq.url ~ "login")) {
    set beresp.ttl = 0s;
    unset beresp.http.Set-Cookie;
    set beresp.uncacheable = true;
    return(deliver);
  }
  if (bereq.method == "GET" && (bereq.url ~ "checkout" || bereq.url ~ "cart")) {
    set beresp.ttl = 0s;
    unset beresp.http.Set-Cookie;
    set beresp.uncacheable = true;
    return(deliver);
